...
`M3D_IMPLEMENTATION`, or if you define `M3D_NOTEXTURE`. This serves two purposes: first, you can avoid code duplication. Second,
you can replace the built-in version with a newer picture decoder should there be any bug.
```c
...
(obviously). If `M3D_NOTEXTURE` is defined, then no stb_image is used and the raw undecoded texture is returned.
...
| `M3D_NOTEXTURE`      | don't load textures, just return their names and raw buffers              |
...
```
Returns the uncompressed / generated textures or NULL (or with `M3D_NOTEXTURE`, `.d` contains the undecoded raw data).
